...
const Cart = () => {
 ...
 const placeOrder = async () => {
  try {
    ...
    if (paymentOption === "COD") {
      ...
      const address = selectedAddresses._id;

      // ✅ Now it's safe to log them
      console.log("Placing COD order with:", { userId, items, address });
  ...
  const getUserAddresses = async () => {
    try {
      console.log("Fetching addresses for userId:", user?._id);
 ...
 useEffect(() => {
  ...
  src={product.images?.[0] || assets.default_image}
                  alt={product.name}
                />
              </div>
              <div>
